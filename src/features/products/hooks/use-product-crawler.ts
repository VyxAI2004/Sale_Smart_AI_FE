...
export const useCrawlSearch = () => {
  ...
  return useMutation({
    mutationFn: (payload: CrawlSearchRequest) => ProductCrawlerApi.crawlSearch(payload),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['products', variables.project_id] });
...
export const useCrawlReviews = () => {
  ...
  return useMutation({
    mutationFn: (payload: CrawlReviewsRequest) => ProductCrawlerApi.crawlReviews(payload),
    onSuccess: (data, variables) => {
      ...
      if (data.status === 'success') {
        toast.success(`Crawled ${data.reviews_crawled || 0} reviews successfully`);
      } else {
