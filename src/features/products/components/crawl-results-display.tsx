...
export function CrawlResultsDisplay({
  ...
  const handleCreateProducts = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(
          `Successfully created ${successCount} product${successCount > 1 ? 's' : ''}`
        )
      ...
      if (errorCount > 0) {
        toast.error(
          `Failed to create ${errorCount} product${errorCount > 1 ? 's' : ''}`
        )
      }
    } catch (error) {
  ...
  return (
    <Card className='mt-4'>
      <CardHeader className='pb-3'>
        ...
        <CardDescription>
          Product URLs found from the search crawl. Select URLs to create
          products.
        </CardDescription>
      </CardHeader>
      {isExpanded && (
        <CardContent>
          <div className='mb-4 flex items-center justify-between'>
            <div className='flex items-center gap-2'>
              <Checkbox
                checked={
                  selectedUrls.size === productUrls.length &&
              ...
              >
                {isCreating ? (
                  ...
                  <>
                    ...
                    Create {selectedUrls.size} Product
