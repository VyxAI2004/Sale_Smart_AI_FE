...
export function ProductTasks({ productId, projectId }: ProductTasksProps) {
  ...
  const handleGenerateTasks = async () => {
    try {
      ...
      const errorMsg = err.response?.data?.detail || err.message || 'Không thể tạo nhiệm vụ';
      setError(errorMsg);
  ...
  const getNextTaskOrder = () => {
    const tasksWithOrder = tasks.filter(t => t.task_order !== null && t.task_order !== undefined);
  ...
  const handleToggleTask = async (taskId: string, currentStatus: string, task: Task) => {
    try {
      ...
      const errorMsg = err.response?.data?.detail || err.message || 'Không thể cập nhật nhiệm vụ';
      ...
      {tasks.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h4 className="text-lg font-semibold mb-2">Chưa có nhiệm vụ</h4>
            <p className="text-sm text-muted-foreground mb-4">
              Nhấn "Tạo Nhiệm vụ AI" để tự động tạo các nhiệm vụ marketing từ phân tích sản phẩm
            </p>
            <Button onClick={handleGenerateTasks} disabled={isGenerating || !projectId}>
        ...
        <div className="space-y-3">
          {[...tasks].sort((a, b) => {
            ...
            if (a.task_order !== null && a.task_order !== undefined && b.task_order !== null && b.task_order !== undefined) {
            ...
            const nextOrder = getNextTaskOrder();
            return (
            <Card key={task.id} className={`hover:shadow-md transition-shadow ${!canCheck && task.status !== 'completed' ? 'opacity-60' : ''}`}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="pt-1">
                    <Checkbox
                      ...
                      disabled={!canCheck}
                      onCheckedChange={() => handleToggleTask(task.id, task.status, task)}
                      className="h-5 w-5"
                      title={!canCheck && task.status !== 'completed' ? `Bạn phải hoàn thành nhiệm vụ thứ ${nextOrder} trước` : ''}
                    />
                  </div>
