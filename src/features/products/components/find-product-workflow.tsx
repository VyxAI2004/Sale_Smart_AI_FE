...
export function FindProductWorkflow({
  projectId,
  onComplete,
}: FindProductWorkflowProps) {
  ...
  const [stepStatuses, setStepStatuses] = useState<
    Record<WorkflowStep, StepStatus>
  >({
  ...
  useEffect(() => {
    if (currentStep === 'crawl' && productUrls.length > 0 && productsData) {
      ...
      const crawledProducts = productsData.items.filter((p) =>
        productUrls.some(
          (url) => p.url === url || p.url?.includes(new URL(url).pathname)
        )
      )
      ...
      if (crawledProducts.length > 0) {
        setStepStatuses((prev) => ({
          ...
          crawl: {
            status: 'completed',
            message: `${crawledProducts.length} products crawled`,
          },
        }))
  ...
  const handleAISearch = async () => {
    ...
    try {
      ...
      setStepStatuses((prev) => ({
        ...
        search: {
          status: 'completed',
          message: `Found ${uniqueUrls.length} search URLs`,
        },
      }))
  ...
  const handleCrawlSearch = async () => {
    ...
    setStepStatuses((prev) => ({
      ...
      crawl: {
        status: 'in-progress',
        message: 'Crawling products and reviews...',
      },
    }))
    ...
    try {
      ...
      setStepStatuses((prev) => ({
        ...
        'paste-url': {
          status: 'completed',
          message: `Found ${urls.length} product URLs`,
        },
        crawl: {
          status: 'in-progress',
          message: `Crawling reviews for ${urls.length} products...`,
        },
      }))
  ...
  return (
    <div className='space-y-6'>
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) => setActiveTab(value as 'manual' | 'auto')}
        className='w-full'
      >
    ...
    <div className='space-y-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Complete workflow to find, crawl, analyze, and calculate trust
            scores for products
          </CardDescription>
        </CardHeader>
      ...
      {aiSearchResult &&
        (currentStep === 'search' || currentStep === 'paste-url') && (
          <AISearchResultsCard
            data={aiSearchResult}
            isLoading={aiSearch.isPending}
            onUseUrl={
              currentStep === 'paste-url' ? handleUseSearchUrl : undefined
            }
          />
        )}
            ...
            <div className='flex items-center justify-center py-8'>
              {stepStatuses.calculate.status === 'in-progress' ? (
                ...
                <div className='flex flex-col items-center gap-2'>
                  <CheckCircle2 className='h-8 w-8 text-green-600' />
                  <p className='text-muted-foreground text-sm'>
                    Trust scores calculated successfully
                  </p>
                </div>
              ) : (
                <p className='text-muted-foreground text-sm'>
                  Waiting for calculation to start...
                </p>
              )}
            </div>
          </CardContent>
          ...
          <CardContent>
            <div className='flex flex-col items-center justify-center gap-4 py-8'>
              ...
              <p className='text-muted-foreground text-center text-sm'>
                All products have been crawled, analyzed, and trust scores have
                been calculated.
              </p>
              <Button
                onClick={() => {
