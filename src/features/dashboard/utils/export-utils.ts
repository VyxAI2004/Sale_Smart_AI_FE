 ...
 */
export async function exportToExcel(dashboard: DashboardResponse, filename?: string) {
  try {
    ...
    const overviewData = [
      ...
      ['Average Trust Score', `${(dashboard.stats.products.averageTrustScore * 100).toFixed(2)}%`],
      [''],
      ['Tasks Statistics', ''],
      ...
      ['Average Trust Score', `${(dashboard.stats.trustScores.average * 100).toFixed(2)}%`],
    ...
    const projectsData = [
      ['Status', 'Count'],
      ...dashboard.charts.projectsByStatus.map(item => [item.name, item.value]),
    ];
    const projectsSheet = XLSXModule.utils.aoa_to_sheet(projectsData);
    XLSXModule.utils.book_append_sheet(workbook, projectsSheet, 'Projects by Status');
    ...
    const reviewsData = [
      ['Sentiment', 'Count'],
      ...dashboard.charts.reviewsBySentiment.map(item => [item.name, item.value]),
    ];
    const reviewsSheet = XLSXModule.utils.aoa_to_sheet(reviewsData);
    XLSXModule.utils.book_append_sheet(workbook, reviewsSheet, 'Reviews by Sentiment');

    // Sheet 5: Products by Platform
    const productsData = [
      ['Platform', 'Count'],
      ...dashboard.charts.productsByPlatform.map(item => [item.name, item.value]),
    ];
    const productsSheet = XLSXModule.utils.aoa_to_sheet(productsData);
    XLSXModule.utils.book_append_sheet(workbook, productsSheet, 'Products by Platform');

    // Sheet 6: Trust Score Distribution
    const trustScoreData = [
      ['Range', 'Count'],
      ...dashboard.charts.trustScoreDistribution.map(item => [item.range, item.count]),
    ];
    const trustScoreSheet = XLSXModule.utils.aoa_to_sheet(trustScoreData);
    XLSXModule.utils.book_append_sheet(workbook, trustScoreSheet, 'Trust Score Distribution');
    ...
    XLSXModule.utils.book_append_sheet(workbook, activitySheet, 'Recent Activity');

    // Generate filename
    const exportFilename = filename || `dashboard-export-${new Date().toISOString().split('T')[0]}.xlsx`;
 ...
 */
export async function exportToPDF(dashboard: DashboardResponse, filename?: string) {
  try {
    ...
    doc.text(`Generated at: ${new Date(dashboard.lastUpdated).toLocaleString()}`, margin, yPos);
    yPos += 15;
    ...
    const overviewRows = [
      ...
      ['Products with Reviews', dashboard.stats.products.withReviews.toString()],
      ['Average Trust Score', `${(dashboard.stats.products.averageTrustScore * 100).toFixed(2)}%`],
    ...
    const projectsRows = [
      ['Status', 'Count'],
      ...dashboard.charts.projectsByStatus.map(item => [item.name, item.value.toString()]),
    ];
    ...
    const tasksRows = [
      ['Status', 'Count'],
      ...dashboard.charts.tasksByStatus.map(item => [item.name, item.value.toString()]),
    ];
    ...
    const reviewsRows = [
      ['Sentiment', 'Count'],
      ...dashboard.charts.reviewsBySentiment.map(item => [item.name, item.value.toString()]),
    ];
    ...
    const productsRows = [
      ['Platform', 'Count'],
      ...dashboard.charts.productsByPlatform.map(item => [item.name, item.value.toString()]),
    ];
    ...
    const trustScoreRows = [
      ['Range', 'Count'],
      ...dashboard.charts.trustScoreDistribution.map(item => [item.range, item.count.toString()]),
    ];
    ...
    // Generate filename
    const exportFilename = filename || `dashboard-export-${new Date().toISOString().split('T')[0]}.pdf`;
