...
export class ProjectApi {
  ...
  static async create(
    ...
    const response = await http.post<ProjectApiResponse>(
      this.BASE_PATH,
      payload
    )
  ...
  static async getById(id: string): Promise<ProjectApiResponse> {
    const response = await http.get<ProjectApiResponse>(
      `${this.BASE_PATH}/${id}`
    )
    return response.data
  }
  ...
  static async list(params?: {
    ...
    const response = await http.get<ProjectListResponse>(this.BASE_PATH, {
      params,
    })
    return response.data
  }
   ...
   */
  static async assignMultipleUsers(
    id: string,
    userIds: string[]
  ): Promise<ProjectApiResponse> {
    const response = await http.post<ProjectApiResponse>(
   ...
   */
  static async removeUsersFromProject(
    id: string,
    userIds: string[]
  ): Promise<ProjectApiResponse> {
    const response = await http.delete<ProjectApiResponse>(
      `${this.BASE_PATH}/${id}/members`,
      {
