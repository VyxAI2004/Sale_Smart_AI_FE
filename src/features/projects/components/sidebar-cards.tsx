...
import { STATUS_OPTIONS, PRODUCT_CATEGORIES, MOCK_USERS } from '../constants/project.constants';
...
export const SidebarCards = ({ formData, onInputChange }: SidebarCardsProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Set the current project status.</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Select value={formData.target_product_category} onValueChange={(value) => onInputChange('target_product_category', value)}>
              <SelectTrigger className="w-full">
            ...
            <p className="text-xs text-muted-foreground">Choose the target product category.</p>
          </div>
        </CardContent>
      </Card>
                  ...
                  return (
                    <SelectItem key={user.id} value={user.id}>
                      <div className="flex items-center space-x-2">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${avatarProps.colorClass}`}>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {getAssignedUsers().map(user => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-medium ${avatarProps.colorClass}`}>
                        {avatarProps.initials}
                      </div>
