...
import { ProjectApi, type ProjectApiResponse, type CreateProjectPayload, type UpdateProjectPayload } from '../api/project-api';
...
export class ProjectService {
   ...
   */
  static async createProject(formData: ProjectFormData): Promise<ProjectApiResponse> {
    try {
      ...
      const payload: CreateProjectPayload = {
        ...
        target_product_category: projectData.target_product_category || undefined,
        // Ensure assigned_to is never empty array
        assigned_to: projectData.assigned_to && projectData.assigned_to.length > 0 
   ...
   */
  static async createDraftProject(formData: ProjectFormData): Promise<ProjectApiResponse> {
    try {
   ...
   */
  static async updateProject(id: string, formData: Partial<ProjectFormData>): Promise<ProjectApiResponse> {
    try {
   ...
   */
  static async updateProjectStatus(id: string, status: string): Promise<ProjectApiResponse> {
    try {
   ...
   */
  static async assignUsersToProject(id: string, userIds: string[]): Promise<ProjectApiResponse> {
    try {
